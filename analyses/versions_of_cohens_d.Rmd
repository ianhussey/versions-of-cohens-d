...
-   Check if CIs are appropriate where they are calculated manually // Flag which packages do not provide CIs, which is a source of error in practice as I would probably construct a Wald CI with the z-statistic for them, which does not seem appropriate for all of them.
...
groundhog.library(packages, groundhog_day, tolerate.R.version = '4.4.0')
...
cohens_d_s_metafor <- function(data){
  ...
  fit <- metafor::escalc(measure = "SMD", 
                m1i = mean_stimulus2, 
                sd1i = sd_stimulus2, 
                n1i = n_stimulus2,
                m2i = mean_stimulus1, 
...
cohens_d_av_metafor <- function(data){
  # nb always applies hedges correction
  require(dplyr)
  require(tibble)
  require(metafor)
  ...
  fit <- metafor::escalc(measure = "SMDH", 
                m1i = mean_stimulus2, 
                sd1i = sd_stimulus2, 
                n1i = n_stimulus2,
                m2i = mean_stimulus1, 
...
cohens_d_s_mbess <- function(data, hedges_correction = TRUE){
  ...
  t <- stats::t.test(score ~ timepoint, data = data)$statistic
...
d_t_dependent <- function(data, hedges_correction = TRUE, r = 0) { # assumes a correlation of 0 for simplicity
  ...
  fit <- stats::t.test(data$score[data$timepoint == "stimulus2"],
...
d_t_independent <- function(data, hedges_correction = TRUE){
  ...
  fit <- stats::t.test(score ~ timepoint, 
                data = data,
  ...
  results <- 
    bind_rows(
      ...
      effectsize::cohens_d(score ~ timepoint, data = data, pooled_sd = FALSE, adjust = hedges_correction) |>
        as_tibble() |>
        mutate(type = "d_av",
               implementation = "{effectsize::cohens_d}",
               hedges_correction = hedges_correction) |>
        dplyr::select(implementation, type, hedges_correction, estimate = Hedges_g, ci_lower = CI_low, ci_upper = CI_high),
      ...
      effectsize::repeated_measures_d(score ~ timepoint | id, data = data, method = "av", adjust = hedges_correction) |> 
        as_tibble() |> 
        mutate(type = "d_av",
               implementation = "{effectsize::repeated_measures_d}",
               hedges_correction = hedges_correction) |>
...
## Some Comments about observed differences withing categories

-   d_z
    -   d_z (lsr) does not use hedges correction, it is not available for paired data
    -   d_z (effectsize) use stats::sd(x_1-x_2) as denominator instead of calculating r manually, uses approximate hedges
    -   d_z (rstatix) uses this hedges correction: d\*(n - 2)/(n - 1.25)
    -   d_z (esc) uses approximate hedges
-   d_av
    -   d_av (metafor) uses exact hedges correction
    -   d_av (effectsize) uses approximate hedges correction
-   d_b
    -   Either use $x_1$ or $x_2$ as reference, which can potentialy give different results
-   d_rm
    -   They seem to agree
-   d_s
    -   They seem to agree, except for the psych package, as discussed in `hedges.rmd`
-   d_t
    -   correspond to $d_z$ (dependent) and $d_s$ (independent) respectively
