...
multiple_cohens_ds_for_dependent_data <- function(data, hedges_correction = TRUE){
  ...
  results <- 
    bind_rows(
      ...
      effectsize::repeated_measures_d(score ~ timepoint | id, data = data, method = "av", adjust = hedges_correction) |> #they use a different hedges correction
        as_tibble() |> 
        mutate(type = "d_av",
               implementation = "{effectsize}",
               hedges_correction = hedges_correction) |>
